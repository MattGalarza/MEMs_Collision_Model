...
function spring(x1, k1, k3, gss, kss)
    ...
    Fsp = -k1 * x1 - k3 * (x1^3)  # Suspension beam force with cubic term
...
function collision(x1, x2, m2, ke, gp)
    ...
    if abs(x2) < gp  # Non-collision regime
        Fc = -ke * (x2 - x1)  # Simple spring force between shuttle and electrode
...
function electrostatic(x1, x2, Qvar, g0, gp, a, e, ep, cp, wt, wb, ke, E, I, Leff, Tf, Tp, N)
    ...
    function Cvar_func(x2)
        ...
        if abs(x2) < gp  # Non-collision regime (translational)
            ...
            Cair_r = ((e * Tf) / (2 * a)) * log((gp - x2 + a * Leff) / (gp - x2))
            Cvar_r = 1 / (2/crl + 1/Cair_r)  # Series combination
            
            # Left side air capacitance
            Cair_l = ((e * Tf) / (2 * a)) * log((gp + x2 + a * Leff) / (gp + x2))
            Cvar_l = 1 / (2/crl + 1/Cair_l)  # Series combination
    ...
    # Electrostatic force, Fe (from paper's energy gradient formula)
    Fe = -(1/2) * (Qvar^2 / Ctotal^2) * dC
...
function CoupledSystem!(dz, z, p, t, current_acceleration)
    ...
    if collision_state == "translational"
        ...
        Fd = damping(z1, z3, z4, p.a, p.c, p.gp, p.Leff, p.Tf, p.eta, p.lambda, p.sigmap)
    else
        x_dot_for_damping = z2  # Use shuttle velocity for collision (rotation)
        Fd = damping(z1, z3, z2, p.a, p.c, p.gp, p.Leff, p.Tf, p.eta, p.lambda, p.sigmap)
    end
    ...
    Fext = p.m1 * current_acceleration
    
    # Effective electrode mass is different during collision
    if collision_state == "rotational"
        effective_m2 = 2 * p.m2  # Increased effective mass during collision
    else
        effective_m2 = p.m2
    end
    
    # Compute derivatives
    dz[1] = z2  # dx1/dt = x1dot
    dz[2] = (Fs + (p.N / 2) * Fc - Fext) / p.m1  # dx1dot/dt
    dz[3] = z4  # dx2/dt = x2dot
    dz[4] = (Fc + Fd + Fe - Fext) / effective_m2  # dx2dot/dt
    dz[5] = (p.Vbias - (z5 / Ctotal)) / p.Rload  # dQvar/dt
    dz[6] = (p.Vbias - z5 / Ctotal - Vout) / (p.Rload * Ctotal)  # dVout/dt
end
end
...
f = 20.0 # Frequency (Hz)
alpha = 3.0 # Applied acceleration constant (dimensionless)
...
for (i, t) in enumerate(sol.t)
    ...
    if collision_state == "translational"
        Fd = AnalyticalModel.damping(z1, z3, z4, p_new.a, p_new.c, p_new.gp, p_new.Leff, p_new.Tf, p_new.eta, p_new.lambda, p_new.sigmap)
    else
        Fd = AnalyticalModel.damping(z1, z3, z2, p_new.a, p_new.c, p_new.gp, p_new.Leff, p_new.Tf, p_new.eta, p_new.lambda, p_new.sigmap)
    end
    push!(Fd_array, Fd)
