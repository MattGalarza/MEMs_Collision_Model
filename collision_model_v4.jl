...
@with_kw mutable struct Params{T<:Real}
    ...
    c1::T = 0.05         # Shuttle damping [N s/m]. CAUTION: 0.05 gives
    ...
    epshs::T = 1e-9           # activation smoothing [m]
...
function spring(x1, p)
    Fsp = -p.k1*x1 - 0.25*p.k3*x1^3
    Fss = abs(x1) <= p.gss ? 0.0 : -p.kss*(abs(x1) - p.gss)*sign(x1)
    # v3.10 hard stop: as-fabricated frame contact at |x1| = ghs (adjustable).
    # Near-rigid Hertz-like wall, C1 via softpos, exact potential gradient.
    u1 = abs(x1) - p.ghs
    dh = softpos(u1, p.epshs)
    sp = 0.5*(1 + u1/sqrt(u1*u1 + p.epshs*p.epshs))
    Fhs = -p.khs*dh^p.phs*sp*sign(x1)
    return Fsp + Fss + Fhs
end
...
# Point budget: the default dt targets ~150k samples over the requested span;
# for microsecond contact features pass an explicit window (t0, t1) with
# dt = 1e-7 rather than zooming a coarse full-span render, or the fine
# structure you zoom into will be the sampling grid, not the physics.

# ===================== SLOW-MOTION CHANNEL ANIMATIONS (v3.9) =============================
# One looping GIF per state / force, drawn as a moving cursor + progressive
# trace over a chosen window, slowed to human speed. GIFs loop automatically,
# giving the repeating-window playback; each file opens in its own viewer
# window alongside the static plots.
# Usage (after the solve):
#   animate_channels(sol, p_new)                       # last full drive cycle,
#                                                      # all 13 channels, ~12 s loops
#   animate_channels(sol, p_new; ch = [:pen, :Fw, :V]) # quick subset
#   animate_channels(sol, p_new; t0 = 0.4830, t1 = 0.4846, seconds = 15)
#                                                      # single burst, ~9000x slow:
#                                                      # THE call for tap anatomy
# Cost note: frames = seconds*fps per channel; 13 channels x 300 frames takes
# a few minutes of GIF encoding -- use the ch subset for quick looks.
function animate_channels(sol, p; t1 = sol.t[end], t0 = t1 - 1/f,
        ch = [:x1, :x1dot, :x2, :x2dot, :Q, :V, :Ct, :pen, :Fs, :Fc, :Fw, :Fd, :Fe],
        seconds = 12.0, fps = 25, dt = nothing, dir = ".")
    nfr = round(Int, seconds*fps)
    dt === nothing && (dt = max((t1 - t0)/max(4000, 4*nfr), 1e-7))
    Wd = sample_window(sol, p, t0, t1; dt = dt)
    n  = length(Wd.t)
    stride = max(1, div(n, 4000))          # light full-window background trace
    ib = 1:stride:n
    slow = seconds/(t1 - t0)
    files = String[]
    for c in ch
        fsym, sc, lb = INSPECT_CH[c]
        y  = getfield(Wd, fsym) .* sc
        lo, hi = extrema(y)
        pad = 0.06*(hi - lo); pad <= 0 && (pad = 0.1*abs(hi) + 1e-12)
        an = @animate for k in 1:nfr
            j = clamp(round(Int, k/nfr*n), 2, n)
            plot(Wd.t[ib] .* 1e3, y[ib]; color = :lightgray, lw = 0.8, label = "",
                 xlabel = "t (ms)", ylabel = lb, ylim = (lo - pad, hi + pad),
                 title = string(c, "   t = ", round(Wd.t[j]*1e3; digits = 3),
                                " ms   (", round(Int, slow), "x slowed)"))
            plot!(Wd.t[1:j] .* 1e3, y[1:j]; color = :dodgerblue, lw = 1.4, label = "")
            scatter!([Wd.t[j]*1e3], [y[j]]; color = :red, ms = 4, label = "")
            c === :pen && hline!([0.0]; ls = :dash, lc = :gray, label = "")
        end
        fn = joinpath(dir, string("anim_", c, ".gif"))
        gif(an, fn; fps = fps)
        push!(files, fn)
        println("wrote ", fn)
    end
...
function visualize_functions(p)
    ...
    v = plot(x1g .* 1e6, [AM.spring(x, p)*1e6 for x in x1g];
             legend = false, xlabel = "x1 (um)", ylabel = "Fs (uN)",
             title = "V1  Suspension + stopper")
    vline!(v, [p.gss*1e6, -p.gss*1e6]; ls = :dash, lc = :gray)
    vline!(v, [p.ghs*1e6, -p.ghs*1e6]; ls = :dot, lc = :red); display(v)
    # V2 coupling spring
...
# ---- capacitance progression: center -> contact -> theoretical closure ----
# This helper is only for plotting the unreachable deep-rotation limit. The
# dynamic model above keeps its a_min safeguard unchanged.
function Ctotal_for_plot(x2, p)
    u = x2 - p.gp
    u <= p.W && return AM.electrostatic(0.0, x2, 0.0, p)[1]

    # Closing side. eta -> 0 at full closure and log1p(eta)/eta -> 1.
    u = clamp(u, 0.0, p.a*p.Leff)
    eta = (p.a*p.Leff - u)/p.h_eff
    phi = abs(eta) < 1e-8 ? 1.0 : log1p(eta)/eta
    Cair_close = (p.e*p.Tf*p.Leff/p.h_eff)*phi

    # Opposite side continues to open.
...
function plot_capacitance_progression(p; reachable_u = 60e-9)
    ...
    Cg = [Ctotal_for_plot(x, p) for x in xg]
    ...
    function ledger(Wd, p; Fext = Fext_input)
        ...
        for i in 1:n
            ...
            Vm = 0.5*p.k1*x1_^2 + (0.25/4)*p.k3*x1_^4 +
                 ...
                 (p.khs/(p.phs + 1))*AM.softpos(abs(x1_) - p.ghs, p.epshs)^(p.phs + 1) +
